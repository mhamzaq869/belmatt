...
class Organization extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->user_model->check_session_data('organization');
    ...
    public function staffs($param1 = "", $param2 = "")
    {
        if ($this->session->userdata('organization_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        if ($param1 == "add") {
            $this->user_model->add_org_staff();
            redirect(site_url('organization/staffs'), 'refresh');
        } elseif ($param1 == "edit") {
            $this->user_model->edit_org_staff($param2);
            redirect(site_url('organization/staffs'), 'refresh');
        } elseif ($param1 == "delete") {
            $this->user_model->delete_user($param2);
            redirect(site_url('organization/staffs'), 'refresh');
        ...
        $page_data['page_name'] = 'staffs';
        $page_data['page_title'] = get_phrase('Staff'); 
        $this->load->view('backend/index', $page_data);
    }

    public function send_credentials($param1 = "")
    {
        if ($this->session->userdata('organization_login') != true) {
            redirect(site_url('login'), 'refresh');
        } 
       
        $this->user_model->genereate_org_user_pass($param1);
        redirect(site_url('organization/users'), 'refresh');
    }
    ...
    function server_side_staffs_data(){
        ...
        foreach($students as $key => $student):
            ...
            $role = $this->db->select('name')->where('id', $student['role_id'])->get('role')->result_array();
 
            $nestedData['key'] = ++$key;
            $nestedData['photo'] = $photo;
            $nestedData['name'] = $name;
            $nestedData['email'] = $email; 
            $nestedData['role'] = (count($role) > 0) ? $role[0]['name'] : ''; 
            $nestedData['action'] = $action.'<script>$("a, i").tooltip();</script>';
            $data[] = $nestedData;
        endforeach;
    ...
    public function get_courses()
    {
        ...
        if (!empty($courses)) {
            foreach ($courses as $key => $row) {
                ...
                $edit_this_course_url = site_url('organization/course_form/course_edit/' . $row['id']);
                $section_and_lesson_url = site_url('organization/course_form/course_edit/' . $row['id']);
                $academic_progress_url = site_url('organization/course_form/course_edit/' . $row['id'].'?tab=academic_progress');
                ...
                if ($row['status'] == 'active') {
                    ...
                    if ($row['user_id'] != $this->session->userdata('user_id')) {
                        ...
                        $course_status_changing_action = "confirm_modal('" . site_url('organization/change_course_status_for_admin/pending/' . $row['id'] . '/' . $category_id . '/' . $instructor_id . '/all/' . $status) . "')";
                    }
                } else {
                    ...
                    if ($row['user_id'] != $this->session->userdata('user_id')) {
                        ...
                        $course_status_changing_action = "confirm_modal('" . site_url('organization/change_course_status_for_admin/active/' . $row['id'] . '/' . $category_id . '/' . $instructor_id . '/all/' . $status) . "')";
                    }
                }



                $delete_course_url = "confirm_modal('" . site_url('organization/course_actions/delete/' . $row['id']) . "')";
    ...
    {
        if ($this->session->userdata('organization_login') != 1)
            redirect(site_url('login'), 'refresh');
    ...
    public function roles($param1 = "", $param2 = "")
    {
        ...
        if ($param1 == "add") {  
            ...
            $this->user_model->add_role(); // PROVIDING TRUE FOR INSTRUCTOR

            $this->session->set_flashdata('flash_message', 'Role successfully created');
            redirect(site_url('organization/roles'), 'refresh');

        } elseif ($param1 == "edit") { 
            $this->user_model->update_role($param2);

            $this->session->set_flashdata('flash_message', 'Role successfully updated');
            redirect(site_url('organization/roles'), 'refresh');
            ...
            $data = $this->user_model->get_role($param2); 
        ...
        $page_data['page_title'] = get_phrase('organization');
        $page_data['roles'] = $this->user_model->all_org_roles();
        $this->load->view('backend/index', $page_data);
    }
    ...
    function get_select2_user_data($default = ""){ 
        $response = array();
        $result = $this->db->where('role_id !=', 1)->where('organization_id', $this->session->userdata('user_id'))->group_start()->like('first_name', $_GET['searchVal'])->or_like('last_name', $_GET['searchVal'])->or_like('email', $_GET['searchVal'])->group_end()->limit(100)->get('users')->result_array();
        if($default != ''){
